<html>
    ...
    <body>
        ...
        if(!$codeProd=="" && !$nameProd=="" && !$cateProd=="" && !$priceProd=="" && !$modelProd=="" && !$_FILES['img']['name']==""){
            ...
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$_FILES['img']['name'])){
                    if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, Precio, Modelo,  Imagen, Nombre", "'$codeProd','$nameProd','$cateProd','$priceProd', '$modelProd','".$_FILES['img']['name']."','$adminProd'")){
                      echo '
                           <img src="../assets/img/correctofull.png" class="center-all-contens">
                           <br>
                           <h3>El producto se agrego a la tienda con exito</h3>
                           <p class="lead text-cente">
                               La pagina se redireccionara automaticamente. Si no es asi haga click en el siguiente boton.<br>
                               <a href="../configAdmin.php" class="btn btn-primary btn-lg">Volver a administracion</a>
                           </p>';
                   }else{
                      echo '
                ...
                echo '
                    <img src="../assets/img/incorrectofull.png" class="center-all-contens">
                    <br>
                    <h3>El Codigo de producto ya esta registrado.<br>Por favor ingrese otro codigo de producto</h3>
                    <p class="lead text-cente">
                        La pagina se redireccionara automaticamente. Si no es asi haga click en el siguiente boton.<br>
                        <a href="../configAdmin.php" class="btn btn-primary btn-lg">Volver a administración</a>
                    </p>';
